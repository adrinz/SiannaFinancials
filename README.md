...
- Stock screener scans the S&P 500: constituents are **fetched** from a public S&P 500 CSV on a TTL (default 24h, see `SQUARE18_SP500_REFRESH_HOURS` in `app/analyst/universe.py`, optional `SQUARE18_SP500_CSV_URL`); a bundled `sp500.json` is used for cold start / offline, with **stale** last-good data if the network feed fails. Movers use batched `yfinance`; earnings use the Nasdaq calendar. The curated 19-ticker list is used when the broad sources are down.
